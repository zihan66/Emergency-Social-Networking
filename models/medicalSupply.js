...
medicalSupplySchema.statics.findAllMedicalSupply = async function () {
  const allMedicalSupply = await this.find({isDeleted: false}).sort({name: 1});
...
medicalSupplySchema.statics.updateMedicalSupplyToReserved = async function (id, receiver) {
  console.log("id", id);
  const medicalSupply = await this.updateOne({ _id: id }, { isReserved: true, receiver: receiver});
  return medicalSupply;
};

medicalSupplySchema.statics.updateMedicalSupplyToNotReserved = async function (id) {
    console.log("id", id);
    const medicalSupply = await this.updateOne({ _id: id }, { isReserved: false, receiver: null});
    return medicalSupply;
};

medicalSupplySchema.statics.findMedicalSupplyByName = async function (searchContent) {
    console.log("MedicalSupplyname", searchContent);
    const medicalSupply = await this.find({ name: { $regex: searchContent }, isDeleted: false});
    return medicalSupply;
};
